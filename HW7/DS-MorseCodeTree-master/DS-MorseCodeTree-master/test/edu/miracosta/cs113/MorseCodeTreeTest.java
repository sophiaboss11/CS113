...
public class MorseCodeTreeTest {
    ...
    private void compareDecodedValues2(String[] morseCode, String[] decodedValue) {
        try {
            for (int i = 0; i < morseCode.length; i ++) {
                String actualDecodedValue = mct.readMorseCodeTree(morseCode[i]);
                assertEquals("Failure for case " + i, decodedValue[i], actualDecodedValue);
            }
        }
        catch (Exception e) { fail("Test failed - an Exception was thrown."); }
    ...
    public void multipleLettersTest() {
        compareDecodedValues2(MULTIPLES_CODE, MULTIPLES_DECODED);
    }
